...
import { Photo, Restaurant, Review } from "@/domain/domain";
...
export default function WriteReviewPage({
  ...
  useEffect(() => {
    const fetchReviewData = async () => {
      if (isEditing && apiService) {
        try {
          const review = await apiService.getRestaurantReview(params.id, reviewId);
          ...
          if (review) {
            ...
            if (review.photos) {
              ...
              setPreviews(review.photos.map(photo => `/api/photos/${photo.url}`));
            }
          }
        } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const newPhotoIds = photos.map(photo => photo.id);
